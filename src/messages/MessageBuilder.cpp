    ...
    const std::shared_ptr<MessageThread> &thread, const MessagePtr &parent)
{
    ...
    const auto firstContentElement = builder->elements.size();
    builder.addWords(content.split(u' '), twitchEmotes, textState);
    if (detail::isAsciiArt(content))
    {
        for (auto elementIndex = firstContentElement;
             elementIndex < builder->elements.size(); ++elementIndex)
        {
            builder->elements[elementIndex]->setAsciiArt();
        }
    }
