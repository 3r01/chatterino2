...
void MessageLayoutContainer::beginAsciiArt()
{
    this->asciiArtWidth_ = ASCII_ART_WIDTH * this->scale_;
}

void MessageLayoutContainer::endAsciiArt()
{
    this->asciiArtWidth_.reset();
}
...
qreal MessageLayoutContainer::remainingWidth() const
{
    auto contentWidth = this->width_ - int(MARGIN.left() * this->scale_) -
                        int(MARGIN.right() * this->scale_);
    if (this->asciiArtWidth_)
    {
        contentWidth = std::min(contentWidth, *this->asciiArtWidth_);
    }
    return (contentWidth -
            (static_cast<int>(this->line_ + 1) == maxUncollapsedLines()
